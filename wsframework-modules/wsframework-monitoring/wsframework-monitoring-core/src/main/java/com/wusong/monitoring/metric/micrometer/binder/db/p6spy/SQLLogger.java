...
public class SQLLogger extends FormattedLogger {
    ...
    public SQLLogger() {
        log = LoggerFactory.getLogger("monitoring.sql");
    }
    ...
    public void logSQL(int logicConnectionId,String underlyingConnection, String now, long elapsed, Category category, String prepared, String sql,
        String url, Loggable loggable) {
        if (sql.startsWith("/* ping */")) {
            // 不打印 select 1 这样的心跳SQL
            return;
        }
        final String msg = strategy.formatMessage(logicConnectionId, now, elapsed, category.toString(), prepared, sql, url);
        String isAutoCommit;
        try {
        ...
        doLog(String.format("autocommit(%s)|readonly(%s)|underlying_con_%s|%s|%s", isAutoCommit, isReadOnly,underlyingConnection, isolation, msg), category);
    }
